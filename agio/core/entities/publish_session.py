...
from agio.core.entities import BaseObject, AWorkspace
...
class APublishSession(BaseObject):
    ...
    def create(cls,
        ...
        session_id = pipe.create_publish_session(
            ...
            status=status,
            comment=comment,
            workspace_settings_id=workspace_settings_id or cls._get_current_settings_workspace_id(),
            data=data
        )
        return cls(pipe.get_publish_session(session_id))

    @classmethod
    def _get_current_settings_workspace_id(cls):
